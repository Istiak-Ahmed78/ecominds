...
class McqController extends GetxController {
  ...
  RxList<MCQModel> macqList = <MCQModel>[].obs;
  RxBool enableEdit = true.obs;
  void loadMCQList(List<MCQModel> mcq) {
    macqList.value = mcq;
    update();
    resetInputs();
  }
  ...
  void resetInputs() {
    answerIndexs.value = List.generate(macqList.length, (index) => null);
    update();
    changeEditabilityState(true);
  }

  void changeEditabilityState(bool st) {
    enableEdit.value = st;
    update();
  }
  ...
  int totalGottenNumber() {
    int t = 0;
    macqList.asMap().entries.forEach((e) {
      if (answerIndexs[e.key] == e.value.answerIndex) {
        t += 1;
      }
    });
    return t;
